...
class DetectAndLocalize(nn.Module):
            ...
            for x in [i for i in range(-m, m + 1, 1)]:
                for y in [j for j in range(-n, n + 1, 1)]:
                    h[index][0][x + 1][y + 1] = torch.exp(
                        -(((x - mu_x) ** 2) / (2. * sigma_x * sigma_x) + ((y - mu_y) ** 2) / (
                                2. * sigma_y * sigma_y)))
        sumh = h.sum()
        if sumh != 0:
            h /= sumh
    ...
    def smoothing_kernels():
        ...
        param_50 = {'mu_x': torch.tensor([-0.0805]), 'mu_y': torch.tensor([-0.270]), 'sigma_x': torch.tensor([1.303]),
                    'sigma_y': torch.tensor([1.396])}
    ...
    def run_code_for_training_with_CrossEntropy_and_MSE_Losses(self, net, noise):
        ...
        optimizer = optim.Adam(net.parameters(), lr=self.dl_studio.learning_rate)
        kernel = DetectAndLocalize.smoothing_kernels()
        kernel = kernel.to(self.dl_studio.device)
        for epoch in range(self.dl_studio.epochs):
            ...
            for i, data in enumerate(self.train_dataloader):
                ...
                # Apply smoothing
                inputs = DetectAndLocalize.apply_filter(inputs, kernel)
                outputs = net(inputs)
    ...
    def run_code_for_testing_detection_and_localization(self, net, noise):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.test_dataloader):
                ...
                # Apply smoothing
                images = DetectAndLocalize.apply_filter(images, kernel)
                outputs = net(images)
                outputs_label = outputs[0]
        ...
        print("Dataset%s Classification Accuracy: %d %%" %
              (noise, 100 * correct / float(total)))
        print("Dataset%s Confusion Matrix" % (noise))
        out_str = "                "
        for j in range(len(self.dataserver_train.class_labels)):
...
for noise in noise_levels:
    if noise == '0':
        train_file = "PurdueShapes5-10000-train.gz"
        test_file = "PurdueShapes5-1000-test.gz"
    else:
        train_file = "PurdueShapes5-10000-train-noise-" + noise + ".gz"
        test_file = "PurdueShapes5-1000-test-noise-" + noise + ".gz"
    ...
    detector.run_code_for_training_with_CrossEntropy_and_MSE_Losses(model, noise)

    # detector.run_code_for_training_with_CrossEntropy_and_BCE_Losses(model)

    detector.run_code_for_testing_detection_and_localization(model, noise)
