                ...
                for i, data in enumerate(self.train_dataloader):
                    ...
                    for j, noise_label in enumerate(temp_outputs):
                        if noise_label > 0:  # If image is detected to have noise of 20%,or 50% or 80%.
                            kernel = DetectAndLocalize.gaussian_filter(3, 1)
                            kernel = kernel.to(self.dl_studio.device)
                            inputs[j] = DetectAndLocalize.apply_filter(inputs[j], kernel)  # Apply smoothing
                        else:
                            pass
                ...
                for i, data in enumerate(self.test_dataloader):
                    ...
                    for j, noise_label in enumerate(temp_outputs):
                        if noise_label > 0:  # If image is detected to have noise of 20%,or 50% or 80%.
                           kernel = DetectAndLocalize.gaussian_filter(3, 1)
                           kernel = kernel.to(self.dl_studio.device)
                           images[j] = DetectAndLocalize.apply_filter(images[j], kernel)  # Apply smoothing
                        else:
                           pass
                        counter[noise_label] += 1
